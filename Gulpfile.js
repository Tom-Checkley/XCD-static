...
// We can proxy any website (and manipulate CSS), or make a
// standalone site (HTML, images, and CSS):

//  - if we're running a proxy site, only generate CSS from Sass,
// by outputting /styles to /css (Gulpfile.js should be within
// the "assets" folder).

//  - if we're running a standlone site, everything (html,
// templates and styles) is in "app" folder and outputs
// to separate "_site"

var config = require('./alloy_config/config.json');
...
// It will automatically find a free port to run from,
// but starting from this port number:
portfinder.basePort = 3000;

/// MAIN TASKS
/// -------------------------------------

// Why 'gulp start'? Why not just 'gulp'?
// -------------------------------------
// We don't have a default task, because Visual Studio automatically
// starts trying to be 'helpful' by detecting gulp, and running it.
// It can be disabled in VS settings, but not on a per-project basis.

// Static Site (with Jeyll):
// -------------------------------------
// "gulp start"
//     <- to run Jekyll
//     <- this ignores /src/scripts/ and copies /src/html/assets/js/ to /dest/
// "gulp start --ts"
//     <- to run with Typescript and Jekyll
//     <- this ignores contents of /src/html/assets/js/ and compiles /src/scripts/

// Proxied Site:
// -------------------------------------
// "gulp start --site www.sitename.test"
//     <- to run as a proxy
// "gulp start --site www.sitename.test -ts"
//     <- to run as a proxy and compile Typescript
// "gulp start --site www.sitename.test --no-notify"
//     <- to run as a proxy and without "Browsersync connected" popup

gulp.task('start', ['t_clear'], function () {
    if(argv.site){
        // Build Sass and browserSync as proxy, doesn't run jekyll
        gulp.start('t_styles_sass', 't_htmly', 't_typescript', 't_babel', 't_browserSyncProxy', 't_watch-files', 't_info');
    } else {
        // Build Sass and browserSync, includes Autoprefixes and Sourcemaps
        gulp.start('t_styles_sass', 't_typescript', 't_babel', 't_browserSyncJekyll', 't_watch-files', 't_watch-JekyllHtml', 't_info');
    }
});
...
// Netlify
gulp.task('netlify', ['t_clear'], function () {
    // Build Sass and browserSync, includes Autoprefixes and Sourcemaps
    gulp.start('t_styles_sass', 't_typescript', 't_babel', 't_jekyll-build');
});
...
var tsProject = typescript.createProject('./alloy_config/tsconfig.json');
...
gulp.task('t_watch-files', function () {
    ...
    gulp.watch(cfg.ts_input_all, {cwd: './'}, ['t_babel']);
});
